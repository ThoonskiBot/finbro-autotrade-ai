...
# === CONFIG ===
SIGNALS_DIR = "signals/"
DEFAULT_TICKER = "SPY"  # fallback if ticker not found in filename
QTY = 10
...
def process_signal_file(file_path):
    try:
        ...
        if df.empty or 'Close' not in df.columns:
            print(f"⚠️ Skipping {file_path} (empty or missing 'Close' column)")
            return
        ...
        ticker = infer_ticker_from_filename(file_path) or DEFAULT_TICKER
