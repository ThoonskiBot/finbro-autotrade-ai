...
if EMAIL_ADDRESS and EMAIL_PASSWORD and TO_EMAIL:
    try:
        msg = EmailMessage()
        msg['Subject'] = f"FINBRO Upgraded Weekly PDF Report - {datetime.now().strftime('%Y-%m-%d')}"
        msg['From'] = EMAIL_ADDRESS
        msg['To'] = TO_EMAIL
        msg.set_content("Attached is the upgraded FINBRO PDF report with journal, alpha, tracker, and heatmap.")
    ...
    try:
        with open(pdf_path, 'rb') as f:
            response = requests.post(
                ...
                data={"chat_id": CHAT_ID, "caption": f"FINBRO Upgraded PDF Report: {latest_pdf}"},
                files={"document": f}
            )
